...
const getLimits = async (rpc, contract, markets) => {
  ...
  await Promise.all(
    markets.map(async (market) => {
      ...
      let minSell = [];
      let minBuy = [];
      buyOrders.rows.forEach((price) =>
        minBuy.push(parseFloat(price.price.split(" ")[0]))
      );
      sellOrders.rows.forEach((price) =>
        minSell.push(parseFloat(price.price.split(" ")[0]))
      );
      ...
      limitData.push({
        ...
        mostExpensiveBuy: buyUpperBound,
        cheapestSale: sellLowerBound,
      });
    })
  );
  return limitData;
